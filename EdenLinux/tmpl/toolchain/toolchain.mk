...
${local}PATH := $(TOOLCHAIN_ROOT_DIR)/bin:$(PATH)

${local}CMDS := CFLAGS="$(TOOLCHAIN_CFLAGS)" CXXFLAGS="$(TOOlCHAIN_CXXFLAGS)" CC="$(TOOLCHAIN_CC) -I $(TOOLCHAIN_ROOT_DIR)/usr/include" CXX="$(TOOLCHAIN_CXX) -I $(TOOLCHAIN_ROOTFS_DIR)/usr/include" AR="$(TOOLCHAIN_AR)" LD="$(TOOLCHAIN_CC)" RANLIB="$(TOOLCHAIN_RANLIB)" READELF="$(TOOLCHAIN_READELF)" STRIP="$(TOOLCHAIN_STRIP)"
