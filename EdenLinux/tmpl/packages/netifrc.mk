...
#Install rule
${py netifrc = MakeRule('$(PACKAGES_ENV)', 'DESTDIR=$(ROOTFS_DIR)', "$(PACKAGES_NETIFRC_BUILD_DIR)", "install", '$(ROOTFS_DIR)/etc/init.d/net.lo', var_name("etc_files", True), var_name("install"))} 

${py netifrc.recipe.insert(2,'($(CP) -R $(PACKAGES_NETIFRC_SRC_DIR) $(PACKAGES_BUILD_DIR)/); \n')}

${netifrc}
#Add to targets
PACKAGES_BUILD_TARGETS += $(PACKAGES_NETIFRC_INSTALL)
