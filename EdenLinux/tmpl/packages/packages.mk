...
${local()}ENV = PATH=$(TOOLCHAIN_PATH) $(TOOLCHAIN_CMDS) CFLAGS="$(TOOLCHAIN_CFLAGS)" CXXFLAGS="$(TOOlCHAIN_CXXFLAGS)" PKG_CONFIG_LIBDIR=$(ROOTFS_DIR)/usr/lib/pkg-config
...
.PHONY: $(${local()}BUILD)
$(${local()}BUILD): $(TOOLCHAIN_INSTALL) $(TARGET_FSH_INSTALL) $(PACKAGES_BUILD_DIR) $(PACKAGES_KERNEL_BUILD) $(PACKAGES_BUSYBOX_BUILD) $(PACKAGES_E2FSPROGS_INSTALL) $(PACKAGES_IANA-ETC_BUILD) $(PACKAGES_ZLIB_INSTALL) $(PACKAGES_NCURSES_INSTALL) $(PACKAGES_NANO_INSTALL) $(PACKAGES_DROPBEAR_BUILD) $(PACKAGES_UDEV_BUILD) $(PACKAGES_BOOTSCRIPTS_HOST-BUILD) $(PACKAGES_BOOTSCRIPTS_BUILD) $(PACKAGES_AUFS-UTIL_BUILD)


${local()}INSTALL := $(PACKAGES_BUILD_DIR)/.installed
$(${local()}INSTALL): $(PACKAGES_KERNEL_INSTALL) $(PACKAGES_BUSYBOX_INSTALL) $(PACKAGES_IANA-ETC_INSTALL) $(PACKAGES_BOOTDEVS_INSTALL) $(PACKAGES_BASECONF_INSTALL) $(PACKAGES_DROPBEAR_INSTALL) $(PACKAGES_BOOTSCRIPTS_INSTALL) $(PACKAGES_UDEV_DEVICES) $(PACKAGES_UDEV_INSTALL) $(PACKAGES_AUFS-UTIL_INSTALL)
