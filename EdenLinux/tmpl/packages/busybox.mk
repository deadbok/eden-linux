...
${Package("$(PACKAGES_BUILD_DIR)/busybox-$(PACKAGES_BUSYBOX_VERSION)", "", "1.21.1", "http://busybox.net/downloads/busybox-$(PACKAGES_BUSYBOX_VERSION).tar.bz2", "$(ROOTFS_DIR)/bin/busybox")}
...
${MakeRule("$(PACKAGES_ENV)", 'ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(ARCH_TARGET)-', "$(PACKAGES_BUSYBOX_BUILD_DIR)", "all", "$(PACKAGES_BUSYBOX_BUILD_DIR)/busybox", "$(PACKAGES_BUSYBOX_CONFIG)", var_name("build"))}	
...
${MakeRule("$(PACKAGES_ENV)", 'ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(ARCH_TARGET)- CONFIG_PREFIX=$(ROOTFS_DIR)', "$(PACKAGES_BUSYBOX_BUILD_DIR)", "install", "$(ROOTFS_DIR)/bin/busybox", "$(PACKAGES_BUSYBOX_BUILD) $(PACKAGES_BUSYBOX_UDHCPC-SCRIPT)", var_name("install"))}
