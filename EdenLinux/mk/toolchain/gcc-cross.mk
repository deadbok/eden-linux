...
${package("$(TOOLCHAIN_BUILD_DIR)/gcc-$(TOOLCHAIN_GCC-CROSS_VERSION)", "$(TOOLCHAIN_BUILD_DIR)/gcc-cross", "4.4.5", "gcc-$(TOOLCHAIN_GCC-CROSS_VERSION).tar.bz2", "ftp://gcc.gnu.org/pub/gcc/releases/gcc-$(TOOLCHAIN_GCC-CROSS_VERSION)/$(TOOLCHAIN_GCC-CROSS_FILE)")}
...
${autoconf('CFLAGS="" CXXFLAGS="" LDFLAGS=$(LDFLAGS)', '--prefix=$(TOOLCHAIN_ROOT_DIR) --host=$(HOST) --target=$(ARCH_TARGET) --with-sysroot=$(ROOTFS_DIR) --disable-nls --enable-shared --disable-multilib --build=$(HOST) --enable-c99 --enable-long-long --with-mpc=$(TOOLCHAIN_ROOT_DIR) --with-mpfr=$(TOOLCHAIN_ROOT_DIR) --with-gmp=$(TOOLCHAIN_ROOT_DIR) --enable-languages=c,c++ --includedir=$(ROTFS_DIR)/usr/include''', "$(TOOLCHAIN_UCLIBC_INSTALL)")} 
