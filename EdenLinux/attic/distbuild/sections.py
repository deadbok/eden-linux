...
class sections(Builder):
    ...
    def write(self):
        """build the Makefiles for all _sections"""
        ...
        for node in self.tree.IterTree():
            ...
            if (isinstance(node, buildtree.section.Section) and
                ...
                if not build == None:
                    #Get build _sections and run through them
                        ...
                        if isinstance(func, buildtree.function.Function):
                            ...
                            if ((name == "download") and
                                ...
                                self.local_variables["unpack_dir"] = ("$" + "build_dir_"
                                                        + _sections[1:len(_sections)] + "/")
                                self.local_variables["current_package_dir"] = ("$" + "build_dir_"
                                                                + _sections[1:len(_sections)] + "/"
                                                                + "$(DIR_"
                                                                + self.expand_name(node).upper()
                                                                + ")")
                                self.local_variables["package_file_dir"] = ("conf/" + _sections[1:len(_sections)]
                                                             + "/"
                                                             + node.name)
                                ...
                                try:
                                    if not func.inline:
                                        ...
                                        if template_filename == None:
                                            template_filename = (_sections[1:len(_sections)] + "-"
                                                                 + func.name
                                                                 + ".mk")
                                        else:
                                ...
                                if func.name.find("clean") > -1:
                                    self.makefile.addPhonyTarget(rule[0],
                                                                 rule[1],
                                                                 rule[2])
                                else:
                                ...
                                if not _sections[1:len(_sections)] in section_targets:
                                    section_targets[_sections[1:len(_sections)]] = dict()
                                if not func.name in section_targets[_sections[1:len(_sections)]]:
                                    section_targets[_sections[1:len(_sections)]][func.name] = ""
                                section_targets[_sections[1:len(_sections)]][func.name] += " " + rule[0]
