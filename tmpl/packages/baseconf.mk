...
${MakeRule("", "DESTDIR=$(ROOTFS_DIR) MODULES=$(KERNEL_MODULES) ROOT_DEVICE=$(ROOT_DEVICE)", "$(PACKAGES_BASECONF_BUILD_DIR)", "install", "$(ROOTFS_DIR)/etc/hosts", var_name("etc_files", True) + " $(PACKAGES_BASECONF_BUILD_DIR)/Makefile", var_name("install"), True)} 
