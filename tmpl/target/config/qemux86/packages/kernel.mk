...
${Package("$(PACKAGES_BUILD_DIR)/linux-$(PACKAGES_KERNEL_VERSION)", "$(PACKAGES_BUILD_DIR)/linux_build", "3.10.27", "http://linux-kernel.uio.no/pub/linux/kernel/v3.0/linux-$(PACKAGES_KERNEL_VERSION).tar.xz", "$(ROOTFS_DIR)/boot/kernel-$(PACKAGES_KERNEL_VERSION)")}
...
${Rule('$(PACKAGES_KERNEL_BUILD_DIR)/arch/$(KERNEL_ARCH)/boot/bzImage', "$(PACKAGES_KERNEL_LINK-SRC) $(PACKAGES_KERNEL_COPY-CONFIG)", rule_var_name=var_name("build"))}
	KBUILD_OUTPUT=$(PACKAGES_KERNEL_BUILD_DIR) $(PACKAGES_ENV) $(MAKE) -C $(PACKAGES_KERNEL_SRC_DIR) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(ARCH_TARGET)- oldconfig
...
ifdef STRIP_TARGET	
	KBUILD_OUTPUT=$(PACKAGES_KERNEL_BUILD_DIR) $(PACKAGES_ENV) $(MAKE) -C $(PACKAGES_KERNEL_SRC_DIR) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(ARCH_TARGET)- modules_install INSTALL_MOD_PATH=$(ROOTFS_DIR) INSTALL_MOD_STRIP=1
	...
	KBUILD_OUTPUT=$(PACKAGES_KERNEL_BUILD_DIR) $(PACKAGES_ENV) $(MAKE) -C $(PACKAGES_KERNEL_SRC_DIR) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(ARCH_TARGET)- modules_install INSTALL_MOD_PATH=$(ROOTFS_DIR)
endif
